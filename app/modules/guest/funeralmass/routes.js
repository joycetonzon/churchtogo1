...
router.get('/', (req, res) => {
    res.render('guest/funeralmass/views/funeralmassreservation',{user: req.session.user});
});

router.get('/listofrequirements', (req, res) => {
    res.render('guest/funeralmass/views/listofrequirements');
});
...
router.post('/', (req, res) => {
    ...
    //    (select int_eventid from tbl_event where int_guestid= ${req.session.int_guestid})
        db.query(queryString2, (err, results, fields) => {
            if (err) throw err;
            return res.redirect('/guest/funeralmass/listofrequirements');
