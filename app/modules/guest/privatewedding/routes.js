...
router.get('/', (req, res) => {
    res.render('guest/privatewedding/views/privateweddingreservation',{user: req.session.user});
});

router.get('/listofrequirements', (req, res) => {
    res.render('guest/privatewedding/views/listofrequirements');
});
...
router.get('/sponsors/:int_reservationid/listofsponsors', (req, res) => {
    ...
    join tbl_weddingevent on tbl_event.int_eventid = tbl_weddingevent.int_eventid 
    where tbl_reservation.int_reservationid ="${req.session.user.int_reservationid}"`
    
        
        db.query(queryString, (err, results, fields) => {
            if (err) throw err;
            console.log('hello', results);
            return res.render('guest/privatewedding/views/listofsponsors',{ sponsors : results, numberOfSponsors: req.query.numberofsponsors })
            });
...
router.get('/sponsors/:int_reservationid', (req, res) => {
    ...
    where tbl_reservation.int_reservationid ="${req.session.user.int_reservationid}"`
        ...
        db.query(queryString, (err, results, fields) => {
            if (err) throw err;
            return res.render('guest/privatewedding/views/sponsors',{ sponsors : results, resid: req.params.int_reservationid })
            });


    // res.render('guest/privatewedding/views/sponsors');
});


router.post('/', (req, res) => {
    // res.redirect('/guest/privatewedding/listofsponsors');
     ...
     var queryString = `INSERT INTO tbl_event(
        ...
        //    (select int_eventid from tbl_event where int_guestid= ${req.session.int_guestid})
            db.query(queryString2, (err, results, fields) => {
                ...
                // return res.redirect('/guest/privatewedding/listofrequirements');
                ...
                db.query(queryString3, (err, results, fields) => {
                    ...
                    //         if (err) throw err;
                    return res.redirect(`/guest/privatewedding/sponsors/${weddingid.insertId}`);
                    //     });
...
router.post('/sponsors/:int_reservationid', (req, res) => {
    ...
    console.log(req.body.numberofsponsors);
    return res.redirect(`/guest/privatewedding/sponsors/${req.session.user.int_reservationid}?sponsors=${req.body.numberofsponsors}`);
 });
